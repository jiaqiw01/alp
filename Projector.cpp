...
int CProjector::SetReps(int reps, CString *errorMessage) {
	num_reps = reps;
	long lRet = AlpSeqControl(m_DeviceID, m_SequenceID, ALP_SEQ_REPEAT, num_reps);
	if (ALP_OK == lRet && NULL != errorMessage)
		*errorMessage = L"Error in setting number of sequence repeats in alpseqcontrol.";
	return lRet;
}
...
int CProjector::SetSeqTiming( IN OUT CTimingEx &timing, CString *errorMessage)
{
	...
	lRet = AlpSeqTiming(m_DeviceID, m_SequenceID, ALP_DEFAULT, timing.PictureTime, 0, ALP_DEFAULT, ALP_DEFAULT);
	if (ALP_OK!=lRet)
	{
		if (NULL != errorMessage)
...
int CProjector::ProjStartContinuous(CString *errorMessage)
{
	long lRet;
	if (num_reps != 0) {
		lRet = AlpProjStart(m_DeviceID, m_SequenceID);
	}
	else {
